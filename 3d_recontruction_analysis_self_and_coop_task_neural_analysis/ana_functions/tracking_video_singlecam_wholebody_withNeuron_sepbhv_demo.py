...
def tracking_video_singlecam_wholebody_withNeuron_sepbhv_demo(bodyparts_locs_camN,output_look_ornot,output_allvectors,output_allangles,lever_loc_both,
...
session_start_time,fps, start_frame, nframes,cameraID,video_file_original,sqr_thres_tubelever,sqr_thres_face,sqr_thres_body,spike_time_data,lfp_filt_sess_aligned,
spike_channels_data,channel_to_depth): 
    import pandas as pd
    import numpy as np
    ...
    video_file = "/gpfs/radev/pi/nandy/jadi_gibbs_data/VideoTracker_SocialInter/3d_recontruction_analysis_self_and_coop_task_data_saved/example_videos_singlecam_wholebody_demo/"+cameraID+"/"+animal1_filename+"_"+animal2_filename+"/"+date_tgt+"_"+animal1_filename+animal2_filename+"_singlecam_wholebody_tracking_withNeuron_sepbhv_demo.mp4"
    ...
    gs=GridSpec(14,4) # 14 rows, 4 columns
    ...
    ax6=fig.add_subplot(gs[9:13,:]) # recording trace
    ...
    with writer.saving(fig, video_file, 100):
        ...
        for iframe in np.arange(iframe_min,iframe_max,1):
            ...
            if clear_frames:
                ...
                ax6.set_yticklabels(list(map(str,np.arange(-64,0,1))))
                ax6.set_xlabel('time (s)',fontsize = 24)
                ax6.set_ylabel('channels',fontsize=15)
                # ax6.set_title('neural activity trace')
                ax6.spines['top'].set_visible(False)
                ax6.spines['right'].set_visible(False)
                # ax6.spines['bottom'].set_visible(False)
            ...
            for ianimal in np.arange(0,nanimals,1):    
                ...
                if (ianimal == 0):
                    for ichannel_iplot in np.arange(0,64,1):
                        ...
                        ichannel_depth = channel_to_depth[1,channel_to_depth[0]==ichannel_iplot][0]
                        #
                        # plot the spikes
                        #
